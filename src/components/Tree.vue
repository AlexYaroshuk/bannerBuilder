    ...
    computed: {
        ...
        isLastChildHovered() {
            return (containerIndex) => {
                if (!this.dragging || this.draggedItemType !== "child")
                    return false;
                const lastChildIndex = this.containers[containerIndex].children.length - 1;
                return (this.dropIndicator === lastChildIndex + 1 &&
                    this.hoveredContainerIndex === containerIndex);
            };
        },
        childSelectedClass() {
